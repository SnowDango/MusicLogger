...
@DatabaseView(
    ...
    value = """select history.id,history.title,history.artist,history.album,history.album_artist as albumArtist,history.media_id as mediaId,history.listening_unix as listeningUnix,history.app as appString,artwork_data.artwork_id as artworkId from `history` left join `artwork_data` on history.album = artwork_data.album and history.album_artist = artwork_data.artist"""
)
